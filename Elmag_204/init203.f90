...
!-----------------------------------------------------------------------------
   ...
   case(1)                     ! 'best-fit' model of Kneiske & Doll
      ...
      open(1,file='Tables/n_bestfit10.dat',status='old')
      do i=1,n_EBL1
         read (1,*) eir(i),anir(i,:)
      enddo
      close(1)     
   ...
   case(2)                     ! 'lower limit' model of Kneiske & Doll
      ...
      open(1,file='Tables/z-IR.dat',status='old')
      do i=1,n_EBL2
         read(1,*)k,z_ir(i)
      enddo
      close(1)     
      open(1,file='Tables/n_lowerlimit10.dat',status='old')
      do i=1,n_EBL1
         read (1,*)eir(i),anir(i,:)
      enddo
      close(1)   
   ...
   case(3)                      ! Franceschini model, arXiv:0805.1841; data taken directly from paper
      ...
      do i=1,n_EBL2
         z_ir(i)=0.2d0*(i-1)
      enddo    
      open(1,file='Tables/n_Fra.dat',status='old')
      do i=1,n_EBL1
         read(1,*)eir(i),anir(i,:)  
      enddo
      close(1)
   ...
   case(4)                      ! Model C from Finke et al. (2010), arXiv:0905.1115; data taken from http://www.phy.ohiou.edu/~finke/EBL/
      ...
      allocate (eir(n_EBL1))
      allocate (anir(n_EBL1,n_EBL2)) 
      open(1,file='Tables/n_Finke.dat',status='old')
      do i=1,n_EBL2
         z_ir(i)=0.01d0*(i-1)
      enddo    
      do i=1,n_EBL1
   ...
   case(5)                     ! Model from Gilmore et al. (2012), arXiv:1104.0671; data taken from physics.ucsc.edu/~joel/EBLdata-Gilmore2012/
      ...
      allocate (eir(n_EBL1))
      allocate (anir(n_EBL1,n_EBL2))
      open(1,file='Tables/z-IR_Gil.dat',status='old')
      do i=1,n_EBL2
         read(1,*)k,z_ir(i)
      enddo
      close(1)     
      open(1,file='Tables/n_Gil.dat',status='old')
      do i=1,n_EBL1
         read (1,*)eir(i),anir(i,:)
      enddo
      close(1)   
   ...
   case(6)                           ! Model from Dominguez et el.arXiv:1007.1459
      n_EBL1 = 50
      n_EBL2 = 17
      allocate (z_ir(n_EBL2))
      allocate (eir(n_EBL1))
      allocate (anir(n_EBL1,n_EBL2))
      open(1,file='Tables/z-IR_Dom.dat',status='old')
      do i=1,n_EBL2
         read(1,*)k,z_ir(i)
      enddo
      close(1)     
      open(1,file='Tables/n_Dom.dat',status='old')
      do i=1,n_EBL1
         read (1,*)eir(i),anir(i,:)
      enddo
      close(1)   
...
!-----------------------------------------------------------------------------
   ...
   case (-1,1)                                                      ! e^+(e^-)
      ...
      if (emin.ge.eirmax) then      !too low energy
         rate_EBL_tab = 0.d0        !(both final particles below Ethr)
         return             
      end if
...
!-----------------------------------------------------------------------------
   ...
   implicit none
   integer i,m
   double precision emin,zz,zmax,emax1,emin1,eg,aktz,denc,wbb_density
   double precision x1(7),a1(7)
...
!=============================================================================!
   ...
   use EBL_fit
   implicit none
   integer ie,iz,je,jz,jzmax
   double precision e,z,zz,we(2),wz(2)
   ...
   case (6)
      ...
      if(z.le.z_ir(1))then
         ...
         do jz=1,16
            ...
            if(jz.eq.16.and.z.le.5.00001) goto 61
         enddo
         write(*,*)'EBL model=',model
         write(*,*)'z=',z,' jz=',jz,' z_ir=',z_ir(jz)
         stop 'wrong jz'
61       jz=min(jz,15)
         wz(2)=(z-z_ir(jz))/(z_ir(jz+1)-z_ir(jz))
         wz(1)=1.d0-wz(2)
...
subroutine banner(n_proc,i)
   ...
   if (i==0) then
      ...
      write(*,*) '!  EBL model 6 contributed by Manuel Mayer               !'
      write(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
